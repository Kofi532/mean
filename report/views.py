...
from users.models import use, sch_reg,act, classn
import pandas as pd
import xlsxwriter
...
def download_sub(request):
    ...
    #ree = [ii for n,ii in enumerate(ree) if ii not in ree[:n]]
    ...
    for t in ree:
        subjects = pd.DataFrame(reportn.objects.all().values().filter(school = sch).filter(stu_id = 'stu_id').filter(level = t))
        if list(subjects) == []:
            mod =reportn(level =t, school = sch, stu_id = 'stu_id')
            mod.save()
            subjects = pd.DataFrame(reportn.objects.all().values().filter(school = sch).filter(stu_id = 'stu_id').filter(level = t))
...
def upload_report(request):
    if "GET" == request.method:
        ...
        df_act = df_act[df_act['school'] == code]
        term = list(df_act['active_term']) 
        term = term[0]
        ...
        ree = [x for x in ree if x != '0']
        ree = [x for x in ree if x != None]
        report30.objects.all().delete()
        report70.objects.all().delete()
...
def report_reg(request): ##add subjects
    ...
    if request.method == 'POST'and form.is_valid():
        ...
        mod =reportn(stu_id = 'stu_id', school = sch,level = level, subjectA=subjectA, subjectB=subjectB, subjectC=subjectC,subjectD=subjectD, subjectE=subjectE, subjectF=subjectF, subjectG=subjectG, subjectH=subjectH, subjectI=subjectI, subjectJ=subjectJ, subjectK=subjectK, subjectL=subjectL )
        #mod.save()
        df = pd.DataFrame(reportn.objects.all().filter(school = sch).filter(stu_id = 'stu_id').filter(level= level))
        if list(df) == []:
            mod.save()
            return render(request, 'thanks.html', {})
        else:
            reportn.objects.all().filter(school = sch).filter(level= level).delete()
            mod.save()
            return render(request, 'thanks.html', {})
    return render(request, 'addsubject.html',{'z':z,'form': form})
...
def report_cards(request):
    ...
    return render(request, 'reportcard.html', {'z': z})    